...
public class QueryStringGenerator {
	...
	public static String createQueryString(int num){
		...
		if(num==0){
			...
			//associees a ces evenements, des deps de ville ou se passe l'evenements et les infos associes

			result+=Outil.createPrefixe();
			result+="SELECT ?nom_user ?user_location ?commentaire ?status ?event_id ?city ?nom_dep ?datetime"+NL+
					"WHERE"+NL+
					"{"+NL+ 
					"?user rdf:type d2rqevent:user."+NL+
			...
			result+=Outil.createPrefixe();
			result+="SELECT ?event_id ?datetime ?nom_dep ?status (COUNT(?participant) AS ?numOfArtists)"+NL+
					"WHERE"+NL+
					"{"+NL+ 
					...
					"}"+NL+
					"GROUP BY ?event_id ?datetime ?nom_dep ?status HAVING(COUNT(?participant) >=3)"+NL+
					"ORDER BY DESC(?nom_dep)"
					;
		}
